...
func get_upgrade_price(type: Enum.Upgrade_Type):
	match type:
		...
		Enum.Upgrade_Type.AddSeedHelper:
			return State.num_seed_helpers * State.num_seed_helpers * 10
		Enum.Upgrade_Type.AddWaterHelper:
			return State.num_water_helpers * State.num_water_helpers * 10
		Enum.Upgrade_Type.AddSunHelper:
			return State.num_sun_helpers * State.num_sun_helpers * 10
		Enum.Upgrade_Type.AddPluckHelper:
			return State.num_pluck_helpers * State.num_pluck_helpers * 10
		Enum.Upgrade_Type.AddAttackHelper:
			return State.num_attack_helpers * State.num_attack_helpers * 10
